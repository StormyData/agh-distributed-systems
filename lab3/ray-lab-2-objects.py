...
size = 10000000
large_array = [random.randint(-1000, 1000) for _ in range(size)]

large_dict = {f"text {i}": large_array[i] for i in range(size)}
...
@ray.remote
def append_value_to_key(d: dict):
    return {f"{k}{v}" for k, v in d.items()}


@ray.remote
def multiply_by_two(l: list):
    return [i * 2 for i in l]


print(ray.get(append_value_to_key.remote(large_dict_ref)))

print(ray.get(multiply_by_two.remote(large_array_ref)))
