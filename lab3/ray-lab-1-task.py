...
@ray.remote
def bubble_sort_dist(table: list):
    bubble_sort(table)


def bubble_sort_local(n: int):
    tables = [list(range(n)) for _ in range(n)]
    for i in range(n):
        random.shuffle(tables[i])
    for i in range(n):
        bubble_sort(tables[i])


def bubble_sort_remote(n: int):
    tables = [list(range(n)) for _ in range(n)]
    for i in range(n):
        random.shuffle(tables[i])
    ray.get([bubble_sort_dist.remote(tables[i]) for i in range(n)])


print("bubble sort local")
cProfile.run("bubble_sort_local(400)")

print("bubble sort remote")
cProfile.run("bubble_sort_remote(400)")
